<?php
class ControllerAddonPhieuchi extends Controller
{
	private $error = array();
	...
	{
		@$this->data['insert'] = @$this->url->http('addon/phieuchi/insert');
		@$this->data['delete'] = @$this->url->http('addon/phieuchi/delete');		
		...
		//for($i=0;$i < @count(@$rows[$i]);$i++)
		{
			...
			@$this->data['datas'][$i]['link_edit'] = @$this->url->http('addon/phieuchi/update&maphieu='.@$this->data['datas'][$i]['maphieu']);
		...
		@$this->id='content';
		@$this->template="addon/phieuchi_list.tpl";
	...
	{		
		...
		@$this->id='content';
		@$this->template='addon/phieuchi_form.tpl';
		@$this->layout=@$this->user->getLayout();
		@$this->render();
	}
